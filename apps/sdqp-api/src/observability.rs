...
mod tests {
    ...
    fn prometheus_output_tracks_status_families() {
        ...
        let output = metrics.render_prometheus("sdqp-api");
        assert!(output.contains("sdqp_http_requests_total{service=\"sdqp-api\"} 3"));
        assert!(output.contains("sdqp_http_responses_2xx_total{service=\"sdqp-api\"} 1"));
        assert!(output.contains("sdqp_http_responses_4xx_total{service=\"sdqp-api\"} 1"));
        assert!(output.contains("sdqp_http_responses_5xx_total{service=\"sdqp-api\"} 1"));
    }
}
